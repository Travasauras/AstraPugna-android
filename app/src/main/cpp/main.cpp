...
// The match lives outside the renderer so it survives the window being torn down and recreated.
...
void handle_cmd(android_app *pApp, int32_t cmd) {
    switch (cmd) {
        case APP_CMD_INIT_WINDOW:
            ...
            pApp->userData = new Renderer(pApp, gGame);
            break;
...
void android_main(struct android_app *pApp) {
    ...
    if (!gGame) gGame = new Game();

    // Register an event handler for Android events
    pApp->onAppCmd = handle_cmd;

    // Mouse wheel zoom (emulator / Chromebook)
